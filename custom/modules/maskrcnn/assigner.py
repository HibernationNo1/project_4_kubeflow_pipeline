...
def bbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6):
    ...
    if is_aligned:
        ...
        if not (rb - lt).is_cuda and (rb - lt).dtype == torch.float16:
            wh = (rb - lt).float().clamp(0, None).half()
        else:  wh = (rb - lt).clamp(0, None)
    ...
    # calculate gious
    # clamp for cpu float16, tensor fp16 has no clamp implementation
    if not (enclosed_rb - enclosed_lt).is_cuda and (enclosed_rb - enclosed_lt).dtype == torch.float16:
            enclose_wh = (enclosed_rb - enclosed_lt).float().clamp(0, None).half()
    else:  enclose_wh = (enclosed_rb - enclosed_lt).clamp(0, None)
...
class MaxIoUAssigner:
    ...
    def assign(self, bboxes, gt_bboxes, gt_bboxes_ignore=None, gt_labels=None):
        ...
        # overlaps.shape: [gt_bboxes.size[0], bboxes.size[0]]
